...
@testset "Helper Macros" begin
    ...
    @testset "Integration: Both macros together" begin
        function build_receptor_model(; name)
            @independent_variables t
            @common_constants

            @parameters begin
                receptors_per_cell = 50000
                cell_count = 1e6
                volume = 1e-3  # Liters
            end

            @variables receptor_conc(t)
            D = Differential(t)

            # Use common constants for unit conversion (must use in equations to be kept)
            initial_conc = (cell_count * receptors_per_cell) / N_Av * nmol_per_mol / volume

            # Observable quantity
            receptor_fraction = receptor_conc / initial_conc

            # Use constants in the equation so MTK keeps them
            eqs = [D(receptor_conc) ~ -receptor_conc / N_Av * nmol_per_mol]

            @named sys = System(eqs, t; observed = [@observed(receptor_fraction)])
            return sys
        end

        sys = build_receptor_model(name = :receptor)
        ...
        obs = ModelingToolkitBase.observed(sys)
        obs_names = [ModelingToolkitBase.getname(eq.lhs) for eq in obs]
        @test :receptor_fraction ∈ obs_names
    end
end
