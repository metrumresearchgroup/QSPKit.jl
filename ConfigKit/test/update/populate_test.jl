...
@testset "Populate Tests" begin
    ...
    @testset "Constants in Expressions" begin
        ...
        # e.g., TV_L0 = TV_mm3_0 / mm3_per_L where mm3_per_L is a constant
        @independent_variables t
        @parameters TV_L0 TV_mm3_0
        @variables Central(t)
        D = Differential(t)

        eqs = [D(Central) ~ -TV_L0 * Central]
        @named sys = System(eqs, t)
        ...
        Parameters:
          ...
          TV_L0:
            value: TV_mm3_0 / mm3_per_L
            description: "Volume in L (computed from constant)"
        Constants:
          mm3_per_L:
            value: 1.0e6
            description: "Conversion factor from mm³ to L"
        Variables:
          Central:
            initial: 100.0
        """
        ...
        try
            ...
            # Verify the computed value: TV_L0 = 1000 / 1e6 = 0.001
            # Access the parameter value from the solution
            tv_l0_val = prob.ps[MTK.parse_variable(sys_simp, "TV_L0")]
            @test isapprox(tv_l0_val, 0.001; rtol=1e-6)
        finally
            rm(path, force=true)
        end
    end
