...
function populate(
    ...
    # Handles namespace prefixes like "gadkar₊Central" matching "Central"
    ...
    # expressions, so a constant used solely in the model (e.g. a molecular weight that
    # appears only in an observed readout like TCE_plasma_ngpermL) is otherwise never
    ...
    for (name_sym, entry) in keyfile.Parameters
        ...
        if target_obj !== nothing
            ...
            # Use namespace-aware matching (handles gadkar₊param matching param)
    ...
    for (name_sym, entry) in keyfile.Variables
        ...
        if target_obj !== nothing
            ...
            # Use name_in_set for namespace-aware matching (handles gadkar₊var matching var)
    ...
    if !isempty(algebraic_var_names)
        ...
        for k in keys(new_ics)
            ...
            # Check both exact match and suffix match (for namespaced variables like gadkar₊Km_IL33)
    ...
    # Cache for downstream consumers (BayesKit) that need the pre-compiled system
...
# CONVENIENCE METHODS
# ------------------------------------------------------------------
function populate(system::MTK.AbstractSystem, keyfile_path::String; kwargs...)
    kw_dict = Dict{Symbol, Any}(kwargs)
    overrides = get(kw_dict, :overrides, get(kw_dict, "overrides", Dict()))
    delete!(kw_dict, :overrides)
    delete!(kw_dict, "overrides")
    
    keyfile = load_keyfile(keyfile_path; kw_dict...)
    return populate(system, keyfile; overrides=overrides, kw_dict...)
end
