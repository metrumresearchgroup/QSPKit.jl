...
"""
    parameter_table(kf; metadata=nothing, sections=:all, latex=true)

Build a report-ready table from a ConfigKit keyfile. The `unit` column retains
the unit spelling from the keyfile when available. By default, derived values
in `keyfile_value` are rendered as math-mode LaTeX suitable for
`ShowKit.st_new(...) |> stable`, while `expression` always preserves the
original keyfile text. Pass `latex=false` to keep `keyfile_value` as plain text.
"""
    ...
    for (variant_index, variant) in enumerate(selected_variants)
        ...
        if latex
            for row in eachrow(frame)
                row.role === :derived_parameter || continue
                ismissing(row.expression) && continue
                row.value = _latex_expression(row.expression)
            end
        end
        push!(frames, frame)
    end
