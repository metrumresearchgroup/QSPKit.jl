...
@testset "BookKit" begin
    ...
    @testset "book! and lookup roundtrip" begin
        mktempdir() do dir
            result = Dict(:fitted_params => Dict(:k6 => 0.42), :loss => 0.005)

            br = book!("MU1", :accepted;
                result=result,
                rationale="Converged with good residuals",
                store_dir=dir,
            )
            ...
            @test br.status == :accepted
            @test br.data == result
            @test br.rationale == "Converged with good residuals"

            # lookup roundtrip
            looked = lookup("MU1"; store_dir=dir)
            @test looked isa BookedResult
            @test looked.name == "MU1"
            @test looked.status == :accepted
            @test looked.data == result
            @test looked.rationale == "Converged with good residuals"
        end
    end
    ...
    # Build a minimal modeling-unit project that mirrors the PFZ01501 layout:
    # each unit's run/plot script produces a named result with `cached(...)` and
    # reads upstream results with `latest(name=...)`.
    function _write_fixture_project(root)
        ...
        script("mu/01_baseline/run.jl", """
            \"\"\"
            MU1: Baseline Calibration
            \"\"\"
            using TracKit
            result = cached("MU1_AsthmaBaseline"; output_dir="data/sims") do
                tracked_fit(fr, params)
            end
            """)
        script("mu/02_drug_response/run.jl", """
            \"\"\"
            MU2: Drug Response Calibration
            \"\"\"
            using TracKit
            upstream = latest(name="MU1_AsthmaBaseline", output_dir="data/sims")
            result = cached("MU2_AsthmaDrugResponse"; output_dir="data/sims") do
                tracked_fit(fr, params)
            end
            """)
        script("mu/03_trajectory/run.jl", """
            \"\"\"
            MU3: Trajectory Validation
            \"\"\"
            using TracKit
            upstream = latest(name="MU2_AsthmaDrugResponse", output_dir="data/sims")
            result = cached("MU3_AsthmaTrajectory"; output_dir="data/sims") do
                tracked_fit(fr, params)
            end
            """)
        script("mu/03_trajectory/plot.jl", """
            # MU3 plots
            using TracKit
            upstream = latest(name="MU3_AsthmaTrajectory", output_dir="data/sims")
            plot_result = cached("MU3_Plots"; output_dir="data/sims") do
                tracked_plot(paths)
            end
            """)
        script("mu/04_dupi_neutralizing/run.jl", """
            \"\"\"
            MU4: Dupilumab Neutralizing Mechanism
            \"\"\"
            using TracKit
            upstream = latest(name="MU3_AsthmaTrajectory", output_dir="data/sims")
            result = cached("MU4_DupiNeutralizing"; output_dir="data/sims") do
                tracked_fit(fr, params)
            end
            """)
        script("mu/05_additional_predictions/plot.jl", """
            # MU5: Additional Predictions — prediction only, no fitting
            using TracKit
            upstream = latest(name="MU3_AsthmaTrajectory", output_dir="data/sims")
            plot_result = cached("MU5_AdditionalPredictions"; output_dir="data/sims") do
                tracked_plot(paths)
            end
            """)
        script("mu/06_trispecific/plot.jl", """
            #=
            MU6: Hypothetical Trispecific — prediction only
            =#
            using TracKit
            upstream_mu3 = latest(name="MU3_AsthmaTrajectory", output_dir="data/sims")
            upstream_mu4 = latest(name="MU4_DupiNeutralizing", output_dir="data/sims")
            plot_result = cached("MU6_Trispecific"; output_dir="data/sims") do
                tracked_plot(paths)
            end
            """)
        return root
    ...
    @testset "lineage_graph reproduces the unit DAG" begin
        mktempdir() do dir
            ...
            @test names == Set(["MU1", "MU2", "MU3", "MU4", "MU5", "MU6"])

            edgeset = Set((e.src, e.dst) for e in BookKit.edges(g))
            @test edgeset == Set([("MU1","MU2"), ("MU2","MU3"),
                                  ("MU3","MU4"), ("MU3","MU5"), ("MU3","MU6"),
                                  ("MU4","MU6")])

            # Roots / leaves
            @test BookKit.roots(g) == ["MU1"]
            @test Set(BookKit.leaves(g)) == Set(["MU5", "MU6"])

            # fit vs prediction-only classification
            kind = Dict(n.name => n.kind for n in BookKit.nodes(g))
            @test kind["MU1"] == :fit
            @test kind["MU2"] == :fit
            @test kind["MU3"] == :fit
            @test kind["MU4"] == :fit
            @test kind["MU5"] == :prediction
            @test kind["MU6"] == :prediction

            # descriptions discovered from leading docstrings/comments
            desc = Dict(n.name => n.description for n in BookKit.nodes(g))
            @test occursin("Baseline Calibration", desc["MU1"])
            @test occursin("Trispecific", desc["MU6"])
        end
    end
    ...
    @testset "lineage_graph to_dot" begin
        mktempdir() do dir
            ...
            @test occursin("\"MU1\" -> \"MU2\"", dot)
            @test occursin("\"MU4\" -> \"MU6\"", dot)
    ...
    @testset "lineage_graph result-level granularity" begin
        mktempdir() do dir
            ...
            names = Set(n.name for n in BookKit.nodes(g))
            @test "MU3_AsthmaTrajectory" in names
            @test "MU6_Trispecific" in names
            @test "MU3_Plots" in names

            edgeset = Set((e.src, e.dst) for e in BookKit.edges(g))
            @test ("MU2_AsthmaDrugResponse", "MU3_AsthmaTrajectory") in edgeset
            @test ("MU3_AsthmaTrajectory", "MU6_Trispecific") in edgeset
            @test ("MU4_DupiNeutralizing", "MU6_Trispecific") in edgeset
            # intra-unit edge survives at result granularity (dropped at unit level)
            @test ("MU3_AsthmaTrajectory", "MU3_Plots") in edgeset

            # the binding variable that read the upstream result is captured
            tri = filter(e -> e.dst == "MU6_Trispecific" &&
                              e.src == "MU4_DupiNeutralizing", BookKit.edges(g))
            @test length(tri) == 1
            @test tri[1].via == "upstream_mu4"
        end
    end
    ...
    @testset "lineage_graph from a BookKit store (recorded edges)" begin
        mktempdir() do dir
            clear_consumption!()
            r1 = Dict(:fitted_params => Dict(:k => 1.0), :loss => 0.1)
            book!("MU1", :accepted; result=r1, loss=0.1, store_dir=dir)

            # Auto-captured edge: lookup(MU1) then book!(MU2) records MU1 -> MU2.
            up = lookup("MU1"; store_dir=dir)
            book!("MU2", :accepted; result=Dict(:loss => 0.2), loss=0.2, store_dir=dir)

            # Explicit, labeled edge.
            book!("MU3", :accepted; result="pred",
                  inputs=["MU2" => "drug params"], store_dir=dir)

            store = open_store(dir)
            g = lineage_graph(store)
            edgeset = Set((e.src, e.dst) for e in BookKit.edges(g))
            @test ("MU1", "MU2") in edgeset
            @test ("MU2", "MU3") in edgeset

            # explicit label is preserved
            e23 = first(filter(e -> (e.src, e.dst) == ("MU2", "MU3"), BookKit.edges(g)))
            @test e23.label == "drug params"

            # the store also recorded the rows directly
            row2 = StoreKit.latest_annotation(store.db, "MU2")
            inputs2 = StoreKit.get_result_inputs(store.db, row2.id)
            @test any(i -> i.input_name == "MU1", inputs2)

            clear_consumption!()
        end
    end

    @testset "to_metagraph requires a backend" begin
        mktempdir() do dir
            _write_fixture_project(dir)
            g = lineage_graph(dir)
            @test_throws ErrorException to_metagraph(g)
        end
    end

    @testset "lineage_graph on the PFZ01501 project (if present)" begin
        real = expanduser("~/Projects/Julia/svn-proj-pfz01501")
        if isdir(joinpath(real, "mu"))
            g = lineage_graph(real)
            edgeset = Set((e.src, e.dst) for e in BookKit.edges(g))
            @test ("MU1","MU2") in edgeset
            @test ("MU2","MU3") in edgeset
            @test ("MU3","MU4") in edgeset
            @test ("MU3","MU5") in edgeset
            @test ("MU3","MU6") in edgeset
            @test ("MU4","MU6") in edgeset
            kind = Dict(n.name => n.kind for n in BookKit.nodes(g))
            @test kind["MU1"] == :fit && kind["MU6"] == :prediction
        else
            @info "skipping PFZ01501 integration test (project not present)"
        end
    end

    # ── Phase 1b: structured staleness (upstream / vcs / blob) ──────────────
    ...
    @testset "BookKitSimKitExt: book_extract(::SimContext)" begin
        mktempdir() do dir
            ...
            @test et.status_hint == :accepted
            @test et.payload.format == "BookKit.SimKitPrediction"
            @test et.payload.schema_version == 1
            @test et.payload.kind == :sim_context
            @test isempty(et.payload.phases)
            @test haskey(et.fingerprints, "model")    # Route-3 structural model fp
            ...
            # End-to-end booking stores a portable, data-only prediction rather
            # than the executable MTK/SciML context (which contains generated
            # functions that JLD2 cannot safely round-trip).
            simulated = ctx |> simulate(1.0; saveat=[0.0, 1.0])
            br = @test_logs min_level=Base.CoreLogging.Warn book!("SIMX", simulated; store_dir=dir)
            @test br.status == :accepted
            saved = lookup("SIMX"; store_dir=dir).data
            @test saved.format == "BookKit.SimKitPrediction"
            @test saved.schema_version == 1
            @test saved.kind == :sim_context
            @test length(saved.phases) == 1
            @test saved.phases[1].name == "phase"
            @test Set(column.name for column in saved.phases[1].columns) == Set(["TIME", "_bk_X"])

            population = PopulationResult(
                Subject[],
                Dict{Any,SimContext}("subject-1" => simulated),
                Dict{Any,Exception}(),
            )
            pop_br = @test_logs min_level=Base.CoreLogging.Warn book!("POPSIMX", population; store_dir=dir)
            @test pop_br.status == :accepted
            pop_saved = lookup("POPSIMX"; store_dir=dir).data
            @test pop_saved.kind == :population_result
            @test only(pop_saved.subjects).id == "subject-1"
            @test length(only(pop_saved.subjects).prediction.phases) == 1
            @test isempty(pop_saved.errors)
            store = open_store(dir)
...
# Phase-2 feasibility canary (per-method lowered-IR fingerprinting). Top-level include
# because it defines its own module. See BookKit/docs/staleness_redesign_plan.md §3.
