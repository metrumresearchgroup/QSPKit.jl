# ==============================================================================
# lineage_graph — discover and render a project's compute/dependency DAG
# ==============================================================================
#
# A QSPKit project organizes work into "modeling units" (folders like
# `mu/01_baseline`, `mu/03_trajectory`, …). Each unit's run/plot script writes a
# named, content-addressed result with `cached("MU3_AsthmaTrajectory"; …)` (or
# `book!`) and reads upstream results with `latest(name="MU2_…")` (or `lookup`).
#
...
#      content hash, VCS ref, whether the step fit parameters), reading either a
#      TracKit JLD2 store (`data/sims/*.jld2`) or a BookKit/StoreKit store
...
modeling unit (granularity `:unit`).

# Fields
- `name::String` — result name (e.g. `"MU3_AsthmaTrajectory"`) or unit id (`"MU3"`)
...
- `via::Union{String,Nothing}` — the variable the read was bound to (e.g. `"upstream_mu3"`)
...
A directed acyclic graph of a project's booked results and their consumption
...
- `granularity::Symbol` — `:unit` or `:result`
...
Discover the compute/dependency DAG of a QSPKit `project` and return it as a
...
# Keyword arguments
- `granularity::Symbol` — `:unit` (default) collapses result names sharing a
  `MU<k>` prefix into one modeling-unit node (`MU1`, `MU2`, …) and drops
  intra-unit self-edges; `:result` keeps every booked result as its own node.
- `script_dirs` — directories/files to scan for producer/consumer calls.
  Defaults to `<project>/mu` when it exists, otherwise the whole project root
  (skipping VCS/vendor dirs).
- `store_dirs` — directories holding stored results to read metadata from.
  Defaults to `<project>/data/sims` (TracKit JLD2) plus `<project>` itself
  (BookKit `.provenance/`).
...
roots(g)                         # ["MU1"]
```
For the PFZ01501 asthma project this reproduces the documented DAG
`MU1 → MU2 → MU3`, then `MU3 → {MU4, MU5, MU6}` and `MU4 → MU6`.
...
# revision (TracKit's `cached(...)` layout). We read only file-level metadata —
...
# `MU3_AsthmaTrajectory_166cbecaaa9b4e1c.jld2` -> ("MU3_AsthmaTrajectory", "166cbecaaa9b4e1c")
# The trailing `_<hex>` segment is the content-address fingerprint TracKit appends.
...
Render a [`LineageGraph`] as a Graphviz `dot` string. Fit (calibrated) nodes are
drawn in teal, prediction-only nodes in amber, matching the documented project
diagram. Pipe the result to `dot`, or use [`render_lineage`](@ref).
"""
function to_dot(g::LineageGraph; rankdir::AbstractString="LR",
    ...
    println(io, "// Edges = the latest()/lookup() provenance reads in each unit's scripts.")
...
Convert a [`LineageGraph`] into a `MetaGraphsNext.MetaGraph` (or
`MetaGraphs.MetaDiGraph`) carrying the node and edge metadata as vertex/edge
...
function to_metagraph(g; kwargs...)
    error("to_metagraph requires a Graphs.jl metagraph backend. Run `using MetaGraphs` " *
          "(or `using MetaGraphsNext`) before calling. The LineageGraph returned by " *
