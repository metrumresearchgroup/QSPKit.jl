...
`book!("gof", artifact("figs/gof.png"))` stores the file's bytes as the result
...
provenance metadata. Supports property forwarding so that `result.fitted_params`
works by delegating to the stored data.

# Fields
- `name::String` — the booking name (e.g., "MU2")
- `status::Symbol` — `:accepted` or `:rejected`
- `data::Any` — the actual result (FitResult, DataFrame, etc.)
