...
@testset verbose=true "TargKit matching — SimKit outputs" begin
    ...
    @testset "scan result: keys from scanned values, `at` on solution time" begin
        ...
        ts = TargetSet(DataFrame(dose = [20.0, 10.0], TIME = [3.0, 7.5], Conc = [1.0, 1.0]);
            match = :dose, value = :Conc, at = :TIME)
        ...
        ts_state = TargetSet(DataFrame(dose = [10.0], Central = [1.0]); match = :dose, value = :Central, at = :TIME => 2.0)
    ...
    @testset "same targets against to_dataframe, result(scan), and a SimContext" begin
        ...
        ts = TargetSet(DataFrame(dose = [10.0, 20.0], Conc = [1.0, 1.0]); match = :dose, value = :Conc, at = :TIME => 8.0)
        ...
        single = TargetSet(DataFrame(Conc = [1.0]); value = :Conc, at = :TIME => 8.0)
    ...
    @testset "errors against solutions" begin
        ...
        at5 = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 5.0)
        ...
        outside = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 12.0)
        @test occursin("outside the simulated time span [0.0, 10.0]", msg(outside, results))

        unscanned = TargetSet(DataFrame(dose = [30.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 2.0)
        ...
        between = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 7.5)
        @test occursin("t = 7.5 is not a saved point", msg(between, results))

        no_at = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc)
        @test occursin("needs `at`", msg(no_at, results))

        extra_key = TargetSet(DataFrame(dose = [10.0], donor = [:D1], Conc = [1.0]);
            match = [:dose, :donor], value = :Conc, at = :TIME => 2.0)
        ...
        ambiguous = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 2.0)
    ...
    @testset "failed solves: objective penalty, score error" begin
        ...
        ts = TargetSet(DataFrame(dose = [10.0], Conc = [1.0]); match = :dose, value = :Conc, at = :TIME => 2.0)
    ...
    @testset "fit CL from concentrations at several doses and times" begin
        ...
        ts = TargetSet(DataFrame(rows); match = :dose, value = :Conc, at = :TIME)

        result = fit(ts; simulate = pk_scan, params = [:CL], bounds = (lb = [0.1], ub = [10.0]),
