...
Activate the QSPKit workspace before loading InjecKit:
...
Construct `EventRunner` when the problem and event shape stay fixed:
...
`PreparedEventSolve` also prepares a fixed parameter-update layout for
