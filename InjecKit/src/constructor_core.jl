...
function _build_callback_event_plan(prob::ODEProblem, sys, tspan,
    ...
    # Infusion forcing changes the RHS even when no state/parameter action is
    # scheduled. A callback at each boundary invalidates solver history (in
    # particular CVODE's multistep history); tstops alone do not do that.
    for forcing in state_infusions, time in (forcing.start, forcing.stop)
        time > tstart && get!(runtime_actions, time, _RuntimeAction[])
    end
...
# After mtkcompile, algebraic variables (Complex1(t), Complex2(t)) are moved from
...
function optimized_symbolic_callback_constructor(sys, u0_p_map, tspan, events::Vector{<:MTK.SymbolicDiscreteCallback}; kwargs...)
    ...
    sys_defaults = try Dict{Any,Any}(MTK.get_initial_conditions(sys)) catch; Dict{Any,Any}() end
    # Also include bindings (computed initial conditions like T_cell => total_T_cells_init)
    sys_bindings = try Dict{Any,Any}(MTK.bindings(sys)) catch; Dict{Any,Any}() end
    u0_p_map = _name_aware_merge(sys_defaults, sys_bindings, u0_p_map)
    ...
    # which fails on systems with parameter bindings (e.g., k_growth => expr)
