...
# Ported from TracKit — these functions walk Julia's code_lowered IR to find
...
# ==============================================================================
# Per-method lowered-IR source fingerprinting (Route 2 — the supported Phase-2 API)
# ==============================================================================
#
# Unlike the file-hashing tracer above, this hashes each reachable method's
# *lowered AST* rather than its whole source file. This is the staleness-redesign
# requirement: scientists share `helper.jl` files heavily, so whole-file hashing
# would false-stale every artifact whenever any unrelated function in a shared
# file is edited. Per-method hashing was validated by BookKit's feasibility canary
# (deterministic, insensitive to edits of unused sibling functions, change-
# sensitive, works on REPL-defined methods). Intra-Julia-version only — lowered IR
# layout can shift across versions, so the producing Julia VERSION is captured
# separately (Phase 3).
...
SHA-256 of the method's normalized lowered AST (LineNumberNodes dropped; slot
names included — this is the recipe the feasibility canary validated). Falls back
to a signature-based hash for methods whose AST cannot be introspected.
"""
