...
@testset verbose=true "SimKit" begin
    ...
    @testset "observe()" begin
        ctx = SimContext(base_prob; solver=Tsit5()) |>
            observe(:Central, :Depot) |> simulate(10.0; name=:obs_test)
        @test ctx.observed == [:Central, :Depot]
    end
    ...
    @testset "complex pipeline" begin
        loading = [ev(time=0.0, cmt=:Depot, amt=200.0)]
        maint = [ev(time=0.0, cmt=:Depot, amt=100.0)]

        ctx = SimContext(base_prob; solver=Tsit5()) |>
            with([:ka => 0.5]) |> events(loading) |> keep() |> simulate(days(7); name=:loading) |>
            events(maint) |> keep() |> simulate(weeks(4); name=:maintenance) |>
            simulate(weeks(2); name=:washout)

        @test length(phases(ctx)) == 3
        @test phases(ctx)[1].name == :loading
        @test phases(ctx)[2].name == :maintenance
        @test phases(ctx)[3].name == :washout
        @test phases(ctx)[1].duration == 7.0
        @test phases(ctx)[2].duration == 28.0
        @test phases(ctx)[3].duration == 14.0

        # After washout, params should be cleared (persist was not called)
...
include("test_simulation_options.jl")
