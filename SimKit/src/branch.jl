...
conditions = DataFrame(
    arm = [:mepo, :dupi, :vehicle],
    Emax = [0.99, 0.99, 0.0],
    regimen = [mepo_events, dupi_events, nothing],
)
arms = branch(baseline, conditions; name=:arm, params=[:Emax], event_col=:regimen)
```
"""
...
# Example
```julia
arms = branch_lazy(baseline, :mepo => pipeline1, :dupi => pipeline2)
arms[:mepo]   # pipeline1 runs now, returns SimContext
arms[:dupi]   # pipeline2 runs now
```
"""
...
#     prob |> branch(conditions; name=:arm, params=[:Emax])
