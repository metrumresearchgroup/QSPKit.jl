...
- Required for multi-endpoint estimation (PopKit/BayesKit)
...
function Population(df::DataFrame;
                    ...
                    censoring=nothing,
                    loq=nothing,
                    steady_state::Symbol=:converge)
    steady_state === :converge || throw(ArgumentError("Use steady_state=:converge"))
...
scan(ctx, :igg_baseline => [2.0, 4.0]) do c, p
