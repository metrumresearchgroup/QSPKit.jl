...
```julia
# Simple branch
results = branch(baseline, :low => with(:dose => 10) >> simulate(365.0),
                            :high => with(:dose => 100) >> simulate(365.0))
df = to_dataframe(results)

# Branch with scan inside each arm
results = branch(baseline,
    :SC => scan(:dose => [10, 50], :ii => [7, 14]) do c, p
        c |> events(make_events(p, :Depot)) |> simulate(730.0)
    end,
    :IV => scan(:dose => [100, 500], :ii => [14, 28]) do c, p
        c |> events(make_events(p, :Central)) |> simulate(730.0)
    end,
)
df = to_dataframe(results)
```
...
```julia
# Single-subject scan
sweep = scan(ctx, :lambda => [1, 100, 10000]) do c, p
    c |> with(p) |> simulate(86400.0)
end
df = to_dataframe(sweep; vars=[sys.Tumor_cell], carry_out=[:V1_pkg, :BW])

# Population scan
sweep = scan(ctx, :igg_baseline => [2.0, 4.0, 6.0]) do c, p
    c |> with(p) |> subjects(pop) |> simulate()
end
df = to_dataframe(sweep; obsonly=true, carry_out=[:WT])
```
"""
