# ============================================================
# Pipeline operations: with, events, keep, observe, simulate
# ============================================================
...
function with(ctx::SimContext, pairs)
    ...
    SimContext(ctx.prob, ctx.sol, new_params, ctx.events, ctx.solver,
              ctx.solve_kwargs, ctx.phases, ctx.persist, ctx.observed, ctx.sys)
end
...
function keep(ctx::SimContext)
    SimContext(ctx.prob, ctx.sol, ctx.params, ctx.events, ctx.solver,
              ctx.solve_kwargs, ctx.phases, true, ctx.observed, ctx.sys)
end
...
# ----------------------------------------------------------
# observe — declare output variables of interest
# ----------------------------------------------------------

"""
    observe(ctx::SimContext, vars::Symbol...)

Declare which variables to observe. Affects `result()` output.
"""
function observe(ctx::SimContext, vars::Symbol...)
    SimContext(ctx.prob, ctx.sol, ctx.params, ctx.events, ctx.solver,
              ctx.solve_kwargs, ctx.phases, ctx.persist, collect(vars), ctx.sys)
end

"""
    observe(vars::Symbol...)

Curried form — returns a function `ctx -> observe(ctx, vars...)`.
"""
observe(vars::Symbol...) = PipelineStep(:observe, ctx -> observe(ctx, vars...))
...
function SimRunner(ctx::SimContext, duration::Real; params, absolute::Bool=false,
    ...
    prepared = if has_steady_state(ctx.events)
        base = InjecKit.PreparedEventSolve(prob, InjecKit.IEvent[];
            params=raw_keys, tspan=phase_tspan, base_u0=base_u0)
        PreparedSteadyStateSolve(base, collect(ctx.events))
    else
        InjecKit.PreparedEventSolve(
...
function _simulate_solution(ctx::SimContext, duration::Real; absolute::Bool=false,
    ...
    cached_sol = nothing
    if _cache_enabled()
        cache_key = _cache_key(prob, params, ctx.events, duration, u0, alg, kw)
        cached_sol = _cache_get(cache_key)
    end
...
function simulate(ctx::SimContext, duration::Real; name::Symbol=:phase, absolute::Bool=false,
    ...
    SimContext(prob, sol, new_params, new_events, new_solver, new_solve_kwargs,
              _append_phase(ctx.phases, phase), false, ctx.observed, ctx.sys)
end
