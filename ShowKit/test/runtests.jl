...
@testset "ShowKit" begin
    ...
    @testset "pmtables DataFrame preparation" begin
        original = DataFrame(
            name=[:KD, :KD, :target_per_cell],
            variant=Union{Missing, Symbol}[:control, missing, :AUA1],
            value=[10, 25, 1000],
        )
        prepared = ShowKit._prepare_pmtable_data(original)

        @test prepared.name == ["KD", "KD", "target_per_cell"]
        @test isequal(
            prepared.variant,
            Union{Missing, String}["control", missing, "AUA1"],
        )
        @test prepared.value == original.value
        @test original.name == [:KD, :KD, :target_per_cell]
    end
    ...
    @testset "mrggsave script provenance" begin
        ...
        @test ShowKit._script_name("/Users/me/proj/analysis.jl") == "analysis.jl"
        ...
        @test ShowKit._caller_script() == basename(@__FILE__)
        @test ShowKit._resolve_script(nothing) == basename(@__FILE__)
    end
