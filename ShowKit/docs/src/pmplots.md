...
ShowKit wraps MetrumRG's [pmplots](https://metrumresearchgroup.github.io/pmplots/) package, which provides pre-built pharmacometric diagnostic plots. All functions accept a Julia DataFrame and return a `GGPlot` that can be further customized with `+`.
