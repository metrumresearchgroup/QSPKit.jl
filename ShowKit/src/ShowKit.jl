...
Wraps MetrumRG's pharmacometric plotting and tabling R packages,
...
# R backend status and persistent environment selection
"""
    configure_r!(; mode=:project, verbose=nothing)

Persist the project's R mode and refresh native/R resolutions, reusing unchanged
solutions. `:project` reads pkgr.yml when present;
`:latest` ignores it. A different environment needs a Julia restart if R has
already started. `verbose=true` enables full installer output; `false` restores
concise progress with detailed log files. Omit it to preserve the setting.
See CondaR/README.md for configuration and installation details.
"""
configure_r!(; mode::Symbol=:project, verbose::Union{Nothing,Bool}=nothing) = CondaR.configure!(; mode, verbose)
export r_available, configure_r!
